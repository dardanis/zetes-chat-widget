...
class WidgetChatController extends Controller
{
    ...
    public function sendMessage(Request $request, string $widgetKey): JsonResponse
    {
        ...
        $this->assertWidgetSessionTokenIsValid($session, $payload['session_token']);
    ...
    public function feedback(Request $request, string $widgetKey): JsonResponse
    {
        $project = Project::query()->where('widget_key', $widgetKey)->firstOrFail();
        $this->assertWidgetSecretIsValid($request, $project);
        $this->assertWidgetOriginIsAllowed($request, $project);
        ...
        $message = ChatMessage::query()
            ...
            ->where('id', $payload['chat_message_id'])
        ...
        $feedback = ChatMessageFeedback::query()->updateOrCreate(
            ...
            [
                ...
                'metadata' => [
                    'widget_feedback_at' => now()->toIso8601String(),
                ],
            ]
        );
    ...
    private function assertWidgetSessionTokenIsValid(ChatSession $session, string $sessionToken): void
    {
        $metadata = is_array($session->metadata) ? $session->metadata : [];
        ...
        abort_unless($tokenHash !== '' && Hash::check($sessionToken, $tokenHash), 403, 'Invalid widget session token.');
